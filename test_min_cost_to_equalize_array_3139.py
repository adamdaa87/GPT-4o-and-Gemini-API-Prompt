...
class TestMinCostToEqualizeArray(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        # Define the solution file name using the FILE_EXTENSION variable
        solution_file = f'3139. Minimum Cost to Equalize Array{FILE_EXTENSION}.txt'
    ...
    def run_tests(self):
        ...
        # Define the test cases: input data, cost1, cost2, and expected output
        test_cases = [
            ([4, 1], 5, 2, 15),
            ([2, 3, 3, 3, 5], 2, 1, 6),
            ([3, 5, 3], 1, 3, 4)
        ...
        for i, (input_data, cost1, cost2, expected_output) in enumerate(test_cases, 1):
            start_time = time.perf_counter()  # Start timing
            result = self.solution.minCostToEqualizeArray(input_data, cost1, cost2)
            ...
            print(f"Cost1: {cost1}, Cost2: {cost2}")
    ...
    def test_min_cost_to_equalize_array(self):
